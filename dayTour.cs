    ...
    public partial class dayTour : Form
    {
        ...
        private void comboBoxPackage_Click(object sender, EventArgs e)
        {
            if (textBoxVehicle.Text != string.Empty)
            {
                ...
                DataRow dataRow;
                SqlCommand cmd = new SqlCommand("SELECT Package.ID, Package.Name FROM Package WHERE PackageType = 1 AND VehicleType = "+ vehicle_Type, con);
        ...
        private void buttonCalculate_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxVehicle.Text != string.Empty && textBoxStartKm.Text != string.Empty && textBoxEndKm.Text != string.Empty)
            {
                ...
                SqlCommand rates = new SqlCommand("SELECT Rate.ExtraKmRate, Rate.ExtraHourRate FROM Rate WHERE Package =" + package, con);
                SqlDataReader rateData = rates.ExecuteReader();
                int extraKmRate = 0;
                int extraHourRate = 0;
                while (rateData.Read())
                {
                    extraKmRate = Convert.ToInt32(rateData[0]);
                    extraHourRate = Convert.ToInt32(rateData[1]);
                }

                int hours = (dateTimePickerEndTime.Value.Hour - dateTimePickerStartTime.Value.Hour);
                int waitingCharge = 0;
                ...
                if (hours > maxHours)
                {
                    waitingCharge = (hours - maxHours) * extraHourRate;
                }
                else
                {
                    waitingCharge = 0;
                ...
                float Total = packageCost + waitingCharge + extraKmCharge;

                labelBaseHire.Text = "Base hire charge: LKR " + packageCost;
                labelWaitingCharge.Text = "Waiting charge: LKR "+ waitingCharge;
                labelExtraKmCharge.Text = "Extra Km charge: LKR "+ extraKmCharge;
                labelTotal.Text = "Total hire charge: LKR "+ Total;

                int customer = 1;

                SqlCommand add_cmd = new SqlCommand("INSERT INTO [Hire-DayTour] VALUES(@Customer, @Package, @ExtraKmCharge, @WaitingCharge, @Total)", con);
                ...
                add_cmd.Parameters.AddWithValue("WaitingCharge", waitingCharge);
